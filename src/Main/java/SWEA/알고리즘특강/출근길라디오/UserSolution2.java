...
class UserSolution2 {
    ...
    void init(int N, int M, int mType[], int mTime[]) {
        ...
        segTree = new int[spotN * 4];  // 구간 통과 시간 세그먼트 트리
        updateTree( 1, 1, spotN - 1);
    }
    ...
    int calculate(int mA, int mB) {
        ...
        int ans = caculateT(1, 1, spotN - 1, Math.min(mA, mB) + 1, Math.max(mA, mB));
        return ans;
    }

    private static int updateTree(int node, int s, int e) {
        // s, e : arr 의 시작, 끝 index
        if (s == e) {
            leafNodeIdx[s] = node;
            return segTree[node] = transitT[s];
        }

        int m = (s + e) / 2;
        int a = updateTree(node * 2, s, m);
        int b = updateTree(node * 2 + 1, m + 1, e);
        return segTree[node] = a + b;
    }

    private static int caculateT(int node, int ts, int te, int qs, int qe) {
        if (ts >= qs && qe >= te) {
            return segTree[node];
        }
        ...
        int m = (ts + te) / 2;
        int a = caculateT(node * 2, ts, m, qs, qe);
        int b = caculateT(node * 2 + 1, m + 1, te, qs, qe);
        return a + b;
    }
}