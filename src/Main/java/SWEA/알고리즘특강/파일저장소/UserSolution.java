...
class UserSolution {
    ...
    public int add(int mId, int mSize) {
        System.out.println("add: " + mId + ", size: " + mSize);
        ...
        System.out.println(startIdxs.get(mId).firstKey());

        return startIdxs.get(mId).firstKey();
    }

    public int remove(int mId) {
        System.out.println("remove: sIdx, len" + startIdxs.get(mId));
        int cnt = 0;    // 파일 조각의 수
...
//        TreeMap<Integer, Integer> copy = new TreeMap<>(sIdxs);
    ...
    public int count(int mStart, int mEnd) {
        System.out.println("count: " + mStart + "~" + mEnd);
